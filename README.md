<div align="center">

# Hi 👋, I'm Ahmed Sabry Hamed

### 💻 Computer Science Student | Future Full-Stack Developer | MEAN Stack Learner

<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&size=24&duration=3000&color=36BCF7&center=true&vCenter=true&width=700&lines=Computer+Science+Student;JavaScript+Developer;Future+MEAN+Stack+Developer;Always+Learning+New+Technologies;Welcome+to+my+GitHub+Profile!" />

</div>

---

# 🚀 About Me

🎓 Computer Science Student passionate about software development and problem solving.

💻 I enjoy building responsive web applications and learning modern technologies.

🌱 Currently learning **Node.js**, **Express.js**, **MongoDB**, and the **MEAN Stack**.

🎯 My goal is to become a professional **Full-Stack Developer** and contribute to impactful software projects.

⚡ I believe that consistency and continuous learning are the keys to success.

---

# 🛠️ Tech Stack

### Languages

<p>
<img src="https://skillicons.dev/icons?i=html,css,js,cpp,python" />
</p>

### Frontend

<p>
<img src="https://skillicons.dev/icons?i=html,css,js,bootstrap" />
</p>

### Backend (Currently Learning)

<p>
<img src="https://skillicons.dev/icons?i=nodejs,express,mongodb" />
</p>

### Tools

<p>
<img src="https://skillicons.dev/icons?i=git,github,vscode,linux" />
</p>

---

# 📚 Currently Learning

* Advanced JavaScript
* Node.js
* Express.js
* MongoDB
* REST APIs
* Authentication & Authorization
* Full MEAN Stack Development

---

# 📂 Featured Projects

## 📝 To-Do App

A modern task management application built using **HTML**, **CSS**, and **Vanilla JavaScript**.

### Features

* ✅ CRUD Operations
* 🌙 Dark Mode
* 🔍 Live Search
* 📂 Task Filtering
* ✔️ Complete Tasks
* 📊 Statistics Dashboard
* 💾 Local Storage
* ⌨️ Keyboard Shortcuts
* 📱 Responsive Design

---

# 📈 GitHub Statistics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=ahmed-elesnawy-15&show_icons=true&theme=tokyonight&hide_border=true"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=ahmed-elesnawy-15&layout=compact&theme=tokyonight&hide_border=true"/>

</div>

---

# 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=ahmed-elesnawy-15&theme=tokyonight&hide_border=true"/>

</div>

---

# 🏆 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=ahmed-elesnawy-15&theme=tokyonight&margin-w=10&margin-h=10&no-frame=true"/>

</div>

---

# 📊 Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=ahmed-elesnawy-15&theme=tokyo-night"/>

</div>

---

# 🎯 2026 Goals

* 🚀 Master the MEAN Stack
* 💼 Land a Software Engineering Internship
* 🌍 Build real-world full-stack applications
* ⭐ Contribute to Open Source
* 📚 Strengthen Data Structures & Algorithms
* 🏆 Solve 500+ programming problems

---

# 🌐 Connect With Me

<p align="left">

<a href="https://github.com/ahmed-elesnawy-15">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<!-- Replace with your actual LinkedIn URL -->

<a href="https://www.linkedin.com/in/YOUR-LINKEDIN-USERNAME">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

</p>

---

# 💭 Favorite Quote

> **"Success isn't built in a day. It's built every day."**

---

<div align="center">

### ⭐ Thank you for visiting my profile!

If you like my projects, consider giving them a ⭐.

</div>
